WITH surgical_pts AS
    (SELECT DISTINCT
      ...
      --   AND ( scufx.[PlannedOperationStartInstant] < CONVERT(DATE,DATEADD(DAY,1,CURRENT_TIMESTAMP))
...
WHERE
    ...
    AND (ddim2.[DateValue] < CONVERT(date, surgical_pts.[PlannedOperationStartInstant]) OR ddim2.[DateValue] < CONVERT(date, surgical_pts.[TouchTimeStartInstant]))
    AND surgical_pts.[PlannedOperationStartInstant] IS NOT NULL
    AND ((surgical_pts.[Classification] = 'Elective')
        OR (surgical_pts.[Classification] = 'Expedited (within 2 weeks on elective list)'))

SELECT DISTINCT
 ...
 FROM [CABOODLE_REPORT].[dbo].[SurgicalCaseFact] scf
	...
	LEFT JOIN (
				SELECT PatientDurableKey, booked_destination, booking_date
				FROM (
					SELECT scf.PatientDurableKey, pof.StringResponse booked_destination, poax.OrderedDateKey booking_date,
						ROW_NUMBER() OVER (PARTITION BY scf.PatientDurableKey ORDER BY OrderedDateKey DESC) ranked_order
					FROM [CABOODLE_REPORT].[dbo].[SurgicalCaseFact] scf
					...
					WHERE scf.[PatientDurableKey] >0
						...
						AND pof.SurveyQuestionKey = 1443) ac
				WHERE ac.ranked_order = 1
				) cbd on cbd.[PatientDurableKey]=scf.PatientDurableKey
 WHERE
	scf.[PatientDurableKey] > 0
	AND scf.[PatientDurableKey]> 1 AND scf.[PatientDurableKey] IS NOT NULL

...
WHERE lcrf.[PatientDurableKey] > 1
    ...
    AND lcrf.[ResultInstant] < CONVERT(date, surgical_pts.[PlannedOperationStartInstant])
    AND lcrf.[ResultInstant] > DATEADD(month, -4, CONVERT(date, surgical_pts.[PlannedOperationStartInstant]))
    AND lcd.[Name] IN ('Creatinine', 'Haemoglobin (g/L)', 'White cell count',
                    'Platelet count', 'Sodium', 'Albumin', 'Bilirubin (total)', 'INR', 'C-reactive protein')

WITH surgical_pts AS
    ...
    WHERE scf.[PatientDurableKey] > 0
        ...
        AND dd.[DepartmentName] != 'NHNN THEATRE SUITE' AND dd.[DepartmentName] != 'RNTNE THEATRE SUITE'
        AND patd.[AgeInYears] >= 18
        AND scf.[Classification] IN ('Elective', 'Expedited (within 2 weeks on elective list)')
        AND (wlef.[IntendedManagement] IN ('*Unspecified', 'Inpatient', 'Inpatient Series', 'Night Admit Series')
...
WHERE fvf.[PatientDurableKey] > 1
    ...
    AND fvf.[FirstDocumentedInstant] < CONVERT(date, surgical_pts.[PlannedOperationStartInstant])
    AND fvf.[FirstDocumentedInstant] > DATEADD(month, -4, CONVERT(date, surgical_pts.[PlannedOperationStartInstant]))
    AND fvf.[TakenInstant] < CONVERT(date, surgical_pts.[PlannedOperationStartInstant])
    AND fvf.[TakenInstant] > DATEADD(month, -4, CONVERT(date, surgical_pts.[PlannedOperationStartInstant]))
    AND frd.[Name] IN ('PULSE', 'PULSE OXIMETRY', 'BLOOD PRESSURE', 'RESPIRATIONS',
                        'TEMPERATURE', 'R BAR BMI (CALCULATED)', 'R BMI', 'R AIR OR OXYGEN')

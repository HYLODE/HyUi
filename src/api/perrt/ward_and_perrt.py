...
prediction_df = pd.DataFrame(predictions, index = morning_data.index, columns = ['Admission_probability'])

#Join this to the extra data table
living_sketch = extra_data.merge(prediction_df, left_on = 'hospital_visit_id', right_index = True)
...
shap_value_names = pd.DataFrame(shap_values_df.columns[-np.argsort(shap_values_df, axis = 1)], index = morning_data.index)
df_unsorted = living_sketch.merge(shap_value_names.loc[:, range(5)], left_on = 'hospital_visit_id', right_index = True)
...
df = pd.merge(df, others_df.loc[:, ['hospital_visit_id', 'time_since_perrt_ref']], on = ['hospital_visit_id'])
...
obs_columns = ['Oxygen', 'AVPU', 'SpO2', 'Resp',
       'Pulse', 'Temp', 'NEWS Score', 'Oxygen delivery device',
       'GCS Total', 'Oxygen therapy flow rate', 'NEWS2 Score']
...
extra_obs.loc[extra_obs.vital == 'Temp', 'value'] = (Temp_F.astype(float) - 32)*(5/9)
...
def plot_selected_columns(plot_type: str, patient: int, df: pd.DataFrame) -> None:
    ...
    if plot_type == 'BP':
        ...
        #Map AVPU to numeric 
        df.loc[df.vital == 'AVPU', 'value'] = df.loc[df.vital == 'AVPU', 'value'].map({'A': 5, 'C': 4, 'V': 3, 'P': 2, 'U': 1})
        cols = ['AVPU', 'GCS']
        rangey = [0, 16]
    ...
    #Make the plot
    fig = px.line(plotting_df.loc[plotting_df.hospital_visit_id == int(patient_id),:], 
                  x='observation_datetime', y='value', color='vital', symbol="vital", 
                  width = 700, height = plot_height, color_discrete_map = colors, range_y = rangey, 
                  title = titles)
...
sorted_labs = labs_data.sort_values(['hospital_visit_id', 'observation_datetime', 'vital'], ascending = False) 
sorted_unique_labs = sorted_labs.drop_duplicates(['hospital_visit_id', 'observation_datetime', 'vital'])

#Make a pivot table
pivot_labs = sorted_unique_labs.pivot(index = ['hospital_visit_id', 'observation_datetime'], columns = 'vital', values = ['value'])
...
all_labs = ['HBGL', 'RCC', 'HCT', 'HCTU', 'MCVU', 'PLT', 'NE', 'WCC', 'LY', 
									'NA', 'K', 'CREA', 'GFR', 'UREA',
									'BILI', 'ALP', 'ALT', 'ALB',
									'CRP', 'MG', 'CA', 'CCA', 'PHOS', 'LDH', 'HTRT',
									'INR', 'PT', 'APTT',
...
def results_table(pt_id: int, input_tab: pd.DataFrame, colnames: list, new_names: list = None) -> pd.DataFrame:
    """ function to return a nicely formatted df with lab results"""
    
    #Ensure all the columns are in input_tab
    missing_columns = [colnames[i] for i in np.where(pd.Series(colnames).isin(input_tab.columns) == False)[0]]
    ...
    pt_labs = np.intersect1d(np.where(input_tab.hospital_visit_id == pt_id), np.where(-labs_not_nan))
...
app.layout = html.Div(
    [
        ...
        #Table
        html.Button(id='submit-button-state', n_clicks=0, children='Add/remove selected patients to PERRT list'),
        html.Div(id = 'datatable-row-ids-container'),
        
        #Figure
        html.Div([
            dbc.Row([
                dbc.Col([dcc.Graph(id='obs_graph'),
                dcc.Graph(id='gcs_graph'),
                dcc.Graph(id='temp_graph'),
                dcc.Graph(id = 'oxygen_graph'),
                dcc.Graph(id = 'sats_graph'),
                ]),
                dbc.Col([html.Label('Recent Blood Tests'),
                    html.Div(id = 'LFTs', style={'color': 'black', 'fontSize': 12})])
            ])
...
def generate_table(ncols, search_value, max_rows, dnacpr, perrt):
    ...
    return dbc.Container([
            ...
            dash_table.DataTable(dff.to_dict('records'),[{"name": i, "id": i} for i in dff.iloc[:,1:].columns], id='tbl',
...
@app.callback(Output('obs_graph', 'figure'), 
              Input('tbl', 'active_cell'))
def obs_graph(active_cell):
    ...
    if active_cell:
        ...
        return plot_selected_columns( 'BP', results_df.hospital_visit_id.unique()[1], extra_obs)

@app.callback(Output('oxygen_graph', 'figure'), 
              Input('tbl', 'active_cell'))
def oxygen_graph(active_cell):
    ...
    if active_cell:
        ...
        return plot_selected_columns('Resp', results_df.hospital_visit_id.unique()[1], extra_obs)
...
def temp_graph(active_cell):
    ...
    if active_cell:
        ...
        return plot_selected_columns( 'Temp', results_df.hospital_visit_id.unique()[1], extra_obs)

@app.callback(Output('gcs_graph', 'figure'), 
              Input('tbl', 'active_cell'))
def gcs_graph(active_cell):
    ...
    if active_cell:
        ...
        return plot_selected_columns('GCS', results_df.hospital_visit_id.unique()[1], extra_obs)
 
@app.callback(Output('sats_graph', 'figure'), 
              Input('tbl', 'active_cell'))
def sats_graph(active_cell):
    ...
    if active_cell:
        ...
        return plot_selected_columns('Sats', results_df.hospital_visit_id.unique()[1], extra_obs)
...
def generate_lfts(active_cell):
    ...
    bloods = ['date_time', 'FBC', 'HBGL', 'PLT','WCC', 'NE', 'LY', 'CRP',
            'U+E', 'NA', 'K', 'CREA', 'GFR', 'UREA', 
            'Bone profile', 'MG', 'CCA', 'PHOS' ,
            'LFTs', 'ALB', 'ALP', 'ALT', 'BILI',
            'Clotting', 'INR', 'PT', 'APTT', 
            'Point of care', 'GLU', 
            'Blood gas', 'pH', 'pO2', 'pCO2', 'K+', 'Na+', 'Lac', 'Glu', 'Urea', 'Crea', 'tHb'
            ]
    
    new_names = ['date_time', 'FBC', 'Hb', 'Plt','WCC', 'Neuts', 'Lymph', 'CRP',
            'U+E', 'Na', 'K', 'Cre', 'eGFR', 'Urea', 
            'Bone profile', 'Mg', 'Cor Ca', 'Phos' ,
            'LFTs', 'Alb', 'ALP', 'ALT', 'Bili',
            'Clotting', 'INR', 'PT', 'APTT', 
            'Point of care', 'GLU', 
            'Blood gas', 'pH', 'pO2', 'pCO2', 'K+', 'Na+', 'Lac', 'Glu', 'Urea', 'Crea', 'tHb'
    ...
    return dbc.Container([
            ...
            dash_table.DataTable(table.to_dict('records'),[{"name": i, "id": i} for i in table.columns], id='tbl2',
                                 style_cell={'font-family': 'sans-serif'}),
            dbc.Alert(id='tbl2')
    ])
...
def generate_ues(active_cell):
    ...
    return dbc.Container([
            dbc.Label('U_Es'),
            dash_table.DataTable(table.to_dict('records'),[{"name": i, "id": i} for i in table.columns], id='tbl3',
                                 style_cell={'font-family': 'sans-serif',           
                                            'overflow': 'hidden',

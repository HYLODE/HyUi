...
WITH rough_census AS
       (
        SELECT 	hv.encounter AS csn,
         ...
         WHERE (loc.location_string LIKE 'T03%%' OR loc.location_string LIKE '%%T06PACU%%' OR loc.location_string LIKE '%%GWB L01 CC%%') 
           AND hv.patient_class = 'INPATIENT'
           AND hv.admission_time IS NOT NULL -- check a valid hospital visit
           AND (lv.admission_time >= NOW() - '1 YEAR'::INTERVAL AND lv.admission_time IS NOT NULL) --- '1 DAY'::INTERVAL                                     -- check location admission happened at or before horizon
           --AND (hv.discharge_time IS NULL OR hv.discharge_time > NOW()) -- '1 DAY'::INTERVAL)       -- check patient is still in hospital at horizon
           --AND (lv.discharge_time IS NULL OR lv.discharge_time > NOW())       -- check patient is still at location at horizon
           AND pt.date_of_death IS NOT NULL
			  AND pt.date_of_death >= lv.admission_time
			  AND pt.date_of_death <= lv.discharge_time -- check patient is alive at horizon

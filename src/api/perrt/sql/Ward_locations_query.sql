-- D Stein Oct 2021
-- Query to pull relevant hospital ward locations (by binning all irrelevant locations)
-- Here we are excluding Hospitals: Queens square, Eastman Dental, Macmillan Cancer Centre, Chalfont Epilepsy clinic, Mortimer Market Centre

-- Also binning - anything paediatric, anything outpatient, anything related to ED or ICU, recovery, theatres, IR, endoscopy, anything not actually in the hospital
-- Anthing that appears to be a test or not real location, any patient lounge, chairs etc

SET search_path to star_test, public;

DROP VIEW IF EXISTS flow.ward_location;
CREATE VIEW flow.ward_location AS
...
---Some further notes of things I had thought about but not necessarily included
--- Split 2
--- Thinking about different areas = GWB appears in split 2, as dows WMS, UCHED, RLHIM, presumably also bin anything with Q in front at this stage, anything with OUT
--- Should also bin T06PACU, NICU SPLIT_PART(location_string, '^', 3)

--- Split 3
---I think UTC is the walk in treatment centre - can bin this also majors, anything with chair in it? LOUNGE, MAJ*, OB? Think this would be ED
--- OFF THE FLOOR, OPTHALMOLOGY ROOM, PHYSIO, PLASTER ROOM, RAT, SLEEP@HOME, THEATRE, TR01 - SLEEP STUDIES, TREATMENT ROOM, TREATMENT/ EYE ROOM, TRIAGE ROOM, VIRTUAL, UTC, VT@HOME, WAIT, WAITING, WR, OUT PG, 
--- PAEDIATRIC INFUSION POOL, KITCHEN, DAYCASE, DAY SURGERY POOL, ENDOSCOPY? ENDO?, CLINIC, CLINIC ROOM
--- I think possibly anything with a number ##- is ED?

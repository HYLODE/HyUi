...
-- Query to pull obs and demographics from adult inpatients currently on the wards within a time window
-- Combines the census and obs pull queries (in theory I think this should make it faster
-- Adapted from S Harris example obs query
-- This will flexibly pull relevant obs according to CSN
...
-- First do the census query
{include_outcomes1}, rough_ICU_census AS
       (
        SELECT 	hv.encounter AS csn,
   		hv.hospital_visit_id AS hv_id,
         lv.admission_time    AS admission_dt,
         ...
         WHERE (loc.location_string {icu_location}) --(loc.location_string LIKE 'T03%%' OR loc.location_string LIKE '%%T06PACU%%' OR loc.location_string LIKE '%%GWB L01 CC%%') 
           AND hv.patient_class = 'INPATIENT'
           AND hv.admission_time IS NOT NULL -- check a valid hospital visit
           AND lv.admission_time <= {end_time} + '{delay}'::INTERVAL - '{icu_horizon}'::INTERVAL                                     -- check location admission happened at or before horizon
           AND (hv.discharge_time IS NULL OR hv.discharge_time > {end_time} + '{delay}'::INTERVAL - '{icu_horizon}'::INTERVAL)       -- check patient is still in hospital at horizon
           AND (lv.discharge_time IS NULL OR lv.discharge_time > {end_time} + '{delay}'::INTERVAL)       -- check patient is still at location at horizon
           AND (pt.date_of_death IS NULL OR pt.date_of_death > {end_time} + '{delay}'::INTERVAL) -- check patient is alive at horizon
    ...
    final_ICU_census AS 
        ...
        FROM ICU_census
               LEFT JOIN star.location_visit lv
                     ON ICU_census.hv_id = lv.hospital_visit_id
               LEFT JOIN star.location loc
                     ON lv.location_id = loc.location_id
        WHERE loc.location_string {icu_location} -- LIKE 'T03%%' OR loc.location_string LIKE '%%T06PACU%%' OR loc.location_string LIKE '%%GWB L01 CC%%'
        ORDER BY csn, admission_dt
        ) {include_outcomes2}
...
SELECT
  ...
  CASE 
    ...
    WHEN ot.id_in_application = '401000' THEN 'GCS_V'
    ...
    ,visit.hospital_visit_id AS csn
    ...
    {include_outcomes1} -- choose whether or not we do the query looking for the outcomes not just inputs
    ,CASE 
        WHEN visit.encounter IN (SELECT csn FROM final_ICU_census) THEN CAST(1 AS BIT)
        ELSE CAST(0 AS BIT)
...
WHERE (ob.observation_datetime >= {end_time} - '{obs_horizon}'::INTERVAL AND
...
{specify_location} AND lo.location_string {location}
...
AND visit.encounter IN (SELECT csn from final_census)

ORDER BY csn, ob.observation_datetime DESC
;
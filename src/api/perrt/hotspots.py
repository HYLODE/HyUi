...
for i in hotspots['Ward']:
    ...
    ward_NEWS_over_3 = len(np.intersect1d(np.where(pd.to_numeric(df['news_score']) >=3), np.where(ward_locs)))
    ward_NEWS_over_5 = len(np.intersect1d(np.where(pd.to_numeric(df['news_score']) >=5), np.where(ward_locs)))
    ward_NEWS_over_7 = len(np.intersect1d(np.where(pd.to_numeric(df['news_score']) >=7), np.where(ward_locs)))
    ward_NEWS_over_9 = len(np.intersect1d(np.where(pd.to_numeric(df['news_score']) >=9), np.where(ward_locs)))
    ward_NEWS_over_11 = len(np.intersect1d(np.where(pd.to_numeric(df['news_score']) >=11), np.where(ward_locs)))
    ...
    [
        ...
        html.Div([
            html.Div(children=[
                ...
                dcc.Dropdown([i for i in np.unique(hotspots['Ward'])],
...
def generate_table(search_value, max_rows):
    ...
    (styles, legend) = discrete_background_color_bins(hotspots, columns=['total_icu_admission'])
    ...
    return dbc.Container([
            ...
            legend,
            dash_table.DataTable(hotspots2.to_dict('records'),[{"name": i, "id": i} for i in hotspots2.columns], 
                                 sort_action='native',
                                 id='tbl',

...
from sklearn.metrics import accuracy_score, roc_auc_score, mean_squared_error, mean_absolute_error, auc, confusion_matrix, roc_curve, precision_score, recall_score, f1_score, average_precision_score, log_loss, ConfusionMatrixDisplay
...
class FlexibleSqlQuery:
    ...
    def generate_query(self):
        ...
        if self.type == 'observations':
            #Observations query
            self.query = self.unformatted_query.format(horizon = self.time_interval, #The period over which we pull observations
            ...
            #Labs query
            self.query = self.unformatted_query.format(horizon = self.time_interval, #The period over which we pull results
                                                       end_time = self.end_time, #Time that the prediction is made from (end of obs collection)
            ...
            raise Exception("I don't recognise the type of flexible query you are trying to use")

        
    def observations(self, end_time: str, 
                icu_location: str = "loc.location_string LIKE 'T03%%' OR loc.location_string LIKE '%%T06PACU%%' OR loc.location_string LIKE '%%GWB L01W%%'", 
                min_age: int = 13, location: str = "SELECT location_string FROM ward_location"):
        """
        ...
        if self.type:
            if self.type != 'observations':
                raise Exception("This query is already a obs query. You can't reuse it. Please instantiate a new query")
        else:
            self.type = 'observations'
            
        #Again we are hard coding that census query has to be from this file
        self.unformatted_query = sql_query_from_file('Observations_V3.sql', verbose = False)
        ...
        self.min_age = str(min_age) + ' YEARS' #The minimum age of patients we are looking for
    ...
    def others(self, subtype: str, end_time: str, hv_ids: np.array):
        ...
        if self.type:
            if self.type != 'others':
                raise Exception("This query is already a others query. You can't reuse it. Please instantiate a new query")
        else:
            self.type = 'others'
        ...
        if subtype == 'labs':
            ...
            self.unformatted_query = sql_query_from_file('Consults_V3.sql', verbose = False)
...
def run_pipeline(date_list: list, engine, now: bool = False) -> pd.DataFrame:
    ...
    cols_for_agg = list(['ALB', 'ALP', 'ALT', 'BCST', 'BILI', 'CA', 'CCA', 'COHb', 'CREA', 
                        'CRP', 'FiO2', 'GCS_Total', 'GFR', 'Glu', 'Hb', 'HCTU', 'HTRT', 'INR', 
                        'K', 'LDH', 'LY', 'Lac', 'MCVU', 'MG', 'NA', 'NE', 'NEWS_Score',
                        'Oxygen_therapy_flow_rate', 'PHOS', 'PLT', 'PT', 
                        'Pain_score,_verbal_at_rest', 'Pain_score,_verbal_on_movement', 
                        'Pulse', 'RCC', 'Resp', 'SpO2', 'Temp', 'UREA', 'WCC', 
                        'pCO2', 'pH', 'Sys_BP', 'Dia_BP', 'MAP', 
                        'Oxygen_delivery_device_enc', 'AVPU_enc', 'Oxygen_enc', 
                        'pressure_areas_observed'])

    #Now columns just to take count from
    cols_for_count = list(['Inpatient_consult_to_Intensivist', 'Inpatient_consult_to_PERRT', 'Inpatient_consult_to_Social_Work', 'Recent_palliative_ref'])

    #This are the columns that we only want the most recent value from
    cols_for_last = list(['sex_enc', 'building_enc',
                    'bed_type_enc', 'age_at_obs', 'NEWS2_scale', 'asian_ethnicity', 'black_ethnicity',
                     'chinese_ethnicity', 'other_ethnicity', 'white_ethnicity', 'missing_ethnicity',
                    '10201000174_ward', '1020100163_ward', '1020100172_ward', '1020100175_ward', '1021800004_ward',
                     '1021800025_ward', '1021800026_ward', '1021800027_ward', '1021800028_ward', '1021800030_ward', '1021800031_ward',
                     'ACU_ward', 'BBNU_ward', 'BCN_ward', 'COB_ward', 'F3NU_ward', 'HS15_ward', 'HSDC_ward', 'LW_ward', 'MFAW_ward',
                     'T01_ward', 'T01ECU_ward', 'T06H_ward', 'T07_ward', 'T08N_ward','T08S_ward', 'T09C_ward', 'T09N_ward', 'T09S_ward',
                     'T10O_ward', 'T10S_ward', 'T12N_ward', 'T12S_ward', 'T13N_ward', 'T13S_ward', 'T14N_ward', 'T14S_ward', 'T16N_ward', 
                     'T16S_ward', 'dnacpr', 'recent_surgery', 'ever_surgery', 'recent_icu', 'med_ref', 'og_ref', 'ortho_ref', 'surg_ref',
                     'No_initial_ref', 'onc_ref', 'elective_surgery', 
                    'emergency_surgery', 'emergency_admission', 'icu_admission'])
    
    #Make the colnames
    final_inputs_colnames = list()
    for i in cols_for_agg:
        final_inputs_colnames.extend([f"Mean_{i}", f"Slope_{i}", f"R_{i}", f"Count_{i}", f"Last_{i}"])
    ...
    for number, date in enumerate(date_list):
        ...
        obs_only_df = obs_df.loc[:, ['hospital_visit_id', 'observation_datetime', 'value', 'vital']]
        ...
        labs_df['value'] = labs_df['value_as_real'].combine_first(labs_df['value_as_text'])
        ...
        labs_only_df = labs_df.loc[:, ['hospital_visit_id', 'observation_datetime', 'vital', 'value']]
        ...
        try:
            labs_only_df = labs_only_df.loc[labs_only_df['observation_datetime'] > begin_date,:]
        except:
            locations_before_end = np.array(labs_only_df['observation_datetime'], dtype = np.datetime64)
            labs_only_df = labs_only_df.loc[locations_before_end > np.datetime64(begin_date),:]
        ...
        consults_only_df = consults_df.loc[:, ['hospital_visit_id', 'observation_datetime', 'vital', 'value']]

        #Combine all the palliative ones
        consults_only_df.loc[consults_df.vital == 'Inpatient Consult to Symptom Control and Palliative Care', ['vital']] = 'Recent_palliative_ref'
        consults_only_df.loc[consults_df.vital == 'Inpatient Consult to Transforming end of life care team', ['vital']] = 'Recent_palliative_ref'  
        ...
        results_df = results_df.loc[:,['hospital_visit_id', 'observation_datetime', 'value', 'vital']]

        #Now drop any annoying duplicates
        Unique_results_index = results_df.loc[:, ['hospital_visit_id', 'observation_datetime', 'vital']].drop_duplicates(keep = 'last').index
        ...
        Pivot_results = results_df.pivot(index = ['hospital_visit_id', 'observation_datetime'], columns = 'vital', values = ['value'])
        ...
        Pivot_results['Sys_BP'] = Pivot_results['Dia_BP'] = Pivot_results['MAP'] = np.nan
        ...
        non_pivotted_columns = ['location_string', 'age_at_obs', 'mrn', 'hospital_visit_id',
                                   'ethnicity', 'sex', 'bed', 'ward_raw', 'building',
                                   'bed_type', 'icu_admission', 'hospital_discharge_dt']
        Pivot_all = pd.merge(Pivot_results, obs_df.loc[: ,non_pivotted_columns], on = ['hospital_visit_id'])
        ...
        number_refs = 5 - np.sum([med_refs, surg_refs, og_refs, ortho_refs, haem_onc_refs], axis = 0)
        ...
        last_ref = other_df.loc[num_not_onc > 1, non_onc_refs].apply(argmax_time, axis = 1)
        ...
        haemonc_only = np.intersect1d(np.where(other_df['ref_type'].isna() == True), np.where(other_df.haem_onc_ref.isna() == False))
        ...
        other_df.loc[other_df['ever_surgery'] == '1021800001', 'elective_surgery'] = 1 #GWB theatres
        ...
        ref_cat = np.array(other_df['ref_type']).reshape(-1, 1) #Annoying reshape for sk.OHE
        ...
        other_df = other_df.loc[:, ['hospital_visit_id', 'admission_time', 'dnacpr', 'recent_surgery',
               'ever_surgery', 'recent_icu', 'onc_ref', 'elective_surgery',
               'emergency_surgery', 'emergency_admission']]
        ...
        No_resp_support = np.where(Pivot_all['Oxygen delivery device'] == 'No respiratory support provided')

        #Where FiO2 is 21 or 22 or empty
        FiO2_sealevel = np.where([23 < Pivot_all['FiO2']] == False)
        Room_air_no_support = np.union1d(FiO2_sealevel, np.union1d(Room_air, No_resp_support))

        #Now set to zero
        Pivot_all.loc[Pivot_all.index[np.intersect1d(Room_air_no_support, No_flow_rate)], 'Oxygen therapy flow rate'] = 0        
        ...
        O2_delivery_mapping = {'No respiratory support provided': 0, 
                               'Nasal cannula': 1, 'Simple mask': 1, 'Humidified oxygen mask': 1,
                                'Non-rebreather mask': 1, 'Capno mask': 1,
                               'CPAP/Bi-PAP mask': 2}

        Pivot_all['Oxygen_delivery_device_enc'] = pd.to_numeric(Pivot_all['Oxygen delivery device'].map(O2_delivery_mapping),  errors='coerce')

        #Now encode AVPU
        AVPU_mapping = {'A': 0, 'C': 1, 'V': 2, 'P': 3, 'U': 4}
        Pivot_all['AVPU_enc'] = pd.to_numeric(Pivot_all['AVPU'].map(AVPU_mapping), errors='coerce')

        #Now encode oxygen
        Oxygen_mapping = {'Room air': 0, 'Supplemental Oxygen': 1}
        Pivot_all['Oxygen_enc'] = pd.to_numeric(Pivot_all['Oxygen'].map(Oxygen_mapping),  errors='coerce')
        ...
        if 'NEWS2 Score' in Pivot_all.columns:
            Pivot_all['NEWS Score'] = Pivot_all['NEWS Score'].combine_first(Pivot_all['NEWS2 Score'])
        ...
        ethnicity_mapping = {'White British': 'white',
                             'White Irish': 'white',
        ...
        ethnic_cat = np.array(Pivot_all['ethnic_cat']).reshape(-1, 1) #Annoying reshape for sk.OHE
        OH_enc = OneHotEncoder()
        ...
        nan_ethnicity = [i for i, j in enumerate(ethnic_categories) if j == 'nan_ethnicity']
        ...
        ward_list = np.array(['10201000174', '1020100163', '1020100172', '1020100175',
                               '1021800004', '1021800025', '1021800026', '1021800027',
                               '1021800028', '1021800030', '1021800031', 'ACU', 'BBNU', 'BCN',
                               'COB', 'F3NU', 'HS15', 'HSDC', 'LW', 'MFAW', 'T01', 'T01ECU',
                                'T06H', 'T07', 'T08N', 'T08S', 'T09C', 'T09N', 'T09S', 'T10O',
                               'T10S', 'T12N', 'T12S', 'T13N', 'T13S', 'T14N', 'T14S', 'T16N',
                               'T16S'])
        ...
        OH_ward = OH_enc.transform(np.array(Pivot_all['ward_raw']).reshape(-1, 1)).toarray()
        ...
        Pivot_all['pressure_areas_observed'] = pd.to_numeric(Pivot_all['All pressure areas observed?'].map(Pressure_mapping),  errors='coerce')
        
        
        #Sex
        Sex_mapping = {'F': 0, 'M': 1}
        Pivot_all['sex_enc'] = pd.to_numeric(Pivot_all['sex'].map(Sex_mapping),  errors='coerce')

        #Building
        Building_mapping = {'tower': 0, 'EGA': 1, 'GWB': 2}
        Pivot_all['building_enc'] = pd.to_numeric(Pivot_all['building'].map(Building_mapping),  errors='coerce')

        #Bed type
        Bed_mapping = {'bay': 0, 'sideroom': 1}
        Pivot_all['bed_type_enc'] = pd.to_numeric(Pivot_all['bed_type'].map(Bed_mapping),  errors='coerce')
        ...
        unavailable_columns = [i for i in columns_for_transformed if not i in available_columns]
        ...
        Transformed_dataframe = Pivot_all.loc[:, cols_for_agg + cols_for_last + cols_for_count] 
        non_numeric_columns = Transformed_dataframe.dtypes.index[Transformed_dataframe.dtypes == 'object']
        Transformed_dataframe[non_numeric_columns] = Transformed_dataframe[non_numeric_columns].apply(pd.to_numeric, errors='coerce')

        #Add back CSN and datetime
        Transformed_dataframe[['hospital_visit_id', 'observation_datetime', 'ward_raw']] = Pivot_all[['hospital_visit_id', 'observation_datetime', 'ward_raw']]

        #Set up our new storage
        final_inputs_index = Transformed_dataframe['hospital_visit_id'].unique()
        final_inputs = pd.DataFrame(np.nan, columns = final_inputs_colnames, index = final_inputs_index)

        for colname in cols_for_agg:
    
            all_colnames = [f"Mean_{colname}", f"Slope_{colname}", f"R_{colname}", f"Count_{colname}", f"Last_{colname}"]
    
            #We're looping through all of the columns calling these aggregate functions
            final_inputs.loc[:, all_colnames] = Transformed_dataframe.groupby('hospital_visit_id').agg(
                **{
                    f"Mean_{colname}" : pd.NamedAgg(colname, aggfunc=np.mean), 
       
                    #These two return the slope and r value for a least squares fit line
                    f"Slope_{colname}" : pd.NamedAgg(colname, aggfunc = lambda x: linear_slope(x, returns = 'slope')),
                    f"R_{colname}" : pd.NamedAgg(colname, aggfunc = lambda x: linear_slope(x, returns = 'r value')),
        ...
        for colname in cols_for_last:
            ...
            #We're looping through all of the columns calling these aggregate functions
            final_inputs.loc[:, colname] = Transformed_dataframe.groupby('hospital_visit_id').agg(
                **{colname :  pd.NamedAgg(colname, aggfunc = 'last')
                }
            )
        ...
        for colname in cols_for_count:
            ...
            #We're looping through all of the columns calling these aggregate functions
            final_inputs.loc[:, colname] = Transformed_dataframe.groupby('hospital_visit_id').agg(
                **{colname :  pd.NamedAgg(colname, aggfunc = 'count')
                }
            )
        
        final_inputs.loc[:, 'ward_raw'] = Transformed_dataframe.groupby('hospital_visit_id').agg(
            **{'ward_raw' :  pd.NamedAgg('ward_raw', aggfunc = 'last')
            }
        )
        ...
        for i in final_inputs['ward_raw'].unique():
            ...
            ward_NEWS_over_5 = len(np.intersect1d(np.where(pd.to_numeric(final_inputs['Last_NEWS_Score']) >=5), np.where(ward_locs)))
        ...
        practice_dataset = practice_dataset.append(final_inputs.loc[:, practice_dataset_colnames])

...
#Import my fuctions
from functions import argmax_time, test_function, sql_query_from_file, FlexibleSqlQuery, linear_slope, run_pipeline, pull_extra_data
test_function()
...
if rerun:
    ...
    try:
        print(f'Last run was greater than than 15 minutes ago, at {str(last_run)}: rerunning now')

    except:
        print(f'Last run file unavailable or corrupted, rerunning pipeline now')

    #Run the pipeline
    practice_dataset, obs_df, others_df, results_df, labs_only_df = run_pipeline(list([pd.to_datetime('now').date()]), emapdb_engine, now = True)
    ...
    print(f'Last run was less than 15 minutes ago, at {str(last_run)}: rerunning only extra data')

...
postgresql_url = (
    f"postgresql://sharris9:{settings.UDS_PWD}@{settings.EMAP_DB_HOST}:5432/uds"
)
...
def select_ConsultationRequests():
    ...
    with Session(engine) as session:
        statement = (
            select(Consultation_Request, Consultation_Type)
            .where(
                Consultation_Type.consultation_type_id
                == Consultation_Request.consultation_type_id
            )

...
-- This query is to pull aggregate numbers of labs for each patient
...
--Look at last 72h
WHERE res.stored_from >= NOW() - '3 days'::INTERVAL
-- Only use patients over 16
AND res.valid_from - demog.date_of_birth > '16 YEARS'::INTERVAL

--Only patients on the wards
AND l.location_string IN (SELECT location_string FROM ward_location)

GROUP BY def.test_lab_code, mrn.mrn

...
-- This is designed to be flexible and needs to called from within the Ward_sample_data_generator.ipnyb notebook
...
WHERE ob.observation_datetime > NOW() - '{time}'::INTERVAL

-- Only use patients over 16
AND ob.observation_datetime - demog.date_of_birth > '{age}'::INTERVAL
...
-- This is so can decide whether or not we want to limit the selection from within python
{using} AND ot.id_in_application {not_in} IN ({ob_strings})

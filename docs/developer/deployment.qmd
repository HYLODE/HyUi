...
2. Adding `x-cache-control:no-cache` to the request header will instruct Varnish to skip the cache for the requested endpoint
...
In the absence of an explicit setting, Varnish defaults to a 2 minute TTL for cached objects. This default is overriden in this case and is set to 5 minutes (300 seconds) via response headers via middleware in `api/src/api/main.py`. This is added as a decorator to the `app` and `mock_router` `FastAPI` instances.

TTLs for individual endpoints can be altered by adding `x-cache-control: max-age=<ttl>`, where `<ttl>` represents the desired TTL in seconds.

...
# Experimental PyEnv + Virtualenv + Pip Development Workflow

This is an experimental workflow designed to ultimately align the way packages are installed on development machines and in production. It is ultimately inteded to replace Conda.
...
```sh
> pyenv virtualenv 3.10.7 hyui-web
> pyenv virtualenv 3.10.7 hyui-api
```
...
```sh
> pip install -e code/config
> pip install -e code/utils
> pip install -e code/models
> pip install -e code/web
```
...
```sh
> env $(grep -v '^#' .env | xargs) uvicorn api.main:app --reload
```

...
Notes
- don't attempt joins where there is no room/bed level data (b/c they're not physical
  locations
- you need a multi-key join
    - EMAP 'name' (department.name) joins on to Caboodle 'DepartmentName'
    - EMAP 'bed' (derived by splitting location.location_string) joins on to Caboodle
      'Name'
...
- sometimes (in Caboodle) DepartmentName and Name are duplicated so pick the most
  recently 'created'

Set-up the tables as follows (given an engine/connection)
```python
q = "SELECT lo.location_id,
    lo.location_string,
    lo.department_id,
    lo.room_id,
    lo.bed_id,
    ...
    LEFT JOIN star.department dept
        ON lo.department_id = dept.department_id"
...
- 2022-08-07 approach fails where EMAP does not hold a department/room/bed record
  e.g. `1020200074^C0NCCU HDU SR30^HDU-30 SR` does not exist in EMAP other than as a
  location string
...
def bed_merge(
    ...
    # incomplete list ... but I don't think this is so important if once you've removed
    # non-bed/non-rooms above
    emap_virtual_rooms = [
        "WAITING",
        "CHAIRS",
        "POOL ROOM",
    ...
    dfe = dfe[
        (~dfe["room_hl7"].isin(emap_virtual_rooms)) & (~dfe["department_id"].isnull())
    ]
    ...
    dfe["loc2merge"] = dfe.agg(
        lambda x: f"{x['speciality']} __ {x['department']} __ {x['room']} __ {x['bed']}".lower(),  # noqa
        axis=1,
    )
    print(dfe.loc2merge)
    ...
    dfc = dfc[
        ...
        # excluding OR (Operating Room)
        & (~dfc["DepartmentType"].isin(["OR"]))
    ]
    ...
    dfc["loc2merge"] = dfc.agg(
        lambda x: f"{x['DepartmentSpecialty']} __ {x['DepartmentName']} __ {x['RoomName']} __ {x['Name']}".lower(),  # noqa
        axis=1,
    )
    print(dfc.loc2merge)

...
#     #             histogram, _ = np.histogram(np.random.randn(num_beds),
#                           bins=num_beds)
...
# @callback(
#     Output(ids.ELECTIVES_AGGREGATE_STORE, "data"),
#     Input(ids.STORE_TIMER_1H, "n_intervals"),
# )
# def _store_agg_electives(_: int) -> dict:
#     all_elective_pmf = parse_to_data_frame(
#         requests.get(url=f"{get_settings().api_url}/electives/aggregate/").json(),
#             Abacus,)
#     return [
...
web_stores = html.Div(
    [
        ...
        # dcc.Store(id=ids.ELECTIVES_AGGREGATE_STORE),
    ]
)
